...
import { 
  ...
  List, ArrowLeft, BookOpen, Search, Key, MessageSquare, Banknote
} from 'lucide-react';
import './index.css'; 
...
const App = () => {
  ...
  // 監聽 apiKey 的變化，自動存入 localStorage
  useEffect(() => {
    if (apiKey) {
  ...
  // --- 表單資料 (新增 specialRequests 和 priceRanges) ---
  const [basicData, setBasicData] = useState({
    ...
    specialRequests: '', // 新增：特殊要求
    priceRanges: { high: false, medium: false, low: false } // 新增：價位偏好
  });
  ...
  const handlePriceChange = (e) => {
    ...
    setBasicData(prev => ({
      ...prev,
      priceRanges: { ...prev.priceRanges, [name]: checked }
    }));
  };
  ...
  const generateItinerary = async () => {
    ...
    const priceConstraint = selectedPrices.length > 0 ? selectedPrices.join(', ') : "無限制 (以路線順暢與熱門程度為主)";
    ...
    const systemPrompt = `
      ...
      - Restaurant Budget: ${priceConstraint} (Prioritize restaurants fitting these ranges. If 'None', focus on route convenience).
      
      Requirements:
      1. Logistics: Realistic travel times + buffer. Ensure the route is smooth given the special requests.
  ...
  const renderInputForm = () => (
    <div className="max-w-4xl mx-auto bg-white/90 backdrop-blur-md p-6 md:p-8 rounded-3xl shadow-2xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 border border-white/20">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-5 md:p-6 rounded-2xl border border-blue-100 shadow-inner">
          ...
          <div className="relative">
             <input 
              ...
              className="w-full pl-4 pr-4 py-3 bg-white border border-blue-200 rounded-xl focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none transition-all shadow-sm text-sm md:text-base"
            />
          </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  onChange={handleBasicChange}
                  className="w-full pl-12 p-3 md:p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm md:text-base"
                />
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <select 
                ...
                onChange={handleBasicChange}
                className="w-full p-3 md:p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all appearance-none text-sm md:text-base"
              >
        ...
        <section className="space-y-4">
          <h3 className="text-lg md:text-xl font-bold text-slate-800 flex items-center gap-2">
            <span className="bg-purple-100 p-2 rounded-lg text-purple-600"><MessageSquare className="w-5 h-5" /></span>
            特殊要求與偏好
          </h3>
          
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-600">特殊要求 (指定餐廳、特定區域停留時間等)</label>
            <textarea 
              ...
              rows={2}
              className="w-full p-3 md:p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm md:text-base"
              placeholder="例如：第二天晚上一定要吃燒肉、第三天下午想在天神待久一點購物..."
            />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-600 flex items-center gap-2">
              <Banknote className="w-4 h-4" /> 餐廳價位偏好 (可複選，不勾選代表無限制)
            </label>
            <div className="flex flex-wrap gap-3">
              <label className="flex items-center gap-2 bg-slate-50 px-4 py-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-100 transition-colors">
                <input 
                  type="checkbox" 
                  name="high" 
                  checked={basicData.priceRanges.high} 
                  onChange={handlePriceChange}
                  className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                />
                <span className="text-sm font-medium text-slate-700">高 (NT$1000+)</span>
              </label>
              <label className="flex items-center gap-2 bg-slate-50 px-4 py-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-100 transition-colors">
                <input 
                  type="checkbox" 
                  name="medium" 
                  checked={basicData.priceRanges.medium} 
                  onChange={handlePriceChange}
                  className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                />
                <span className="text-sm font-medium text-slate-700">中 (NT$301-1000)</span>
              </label>
              <label className="flex items-center gap-2 bg-slate-50 px-4 py-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-100 transition-colors">
                <input 
                  type="checkbox" 
                  name="low" 
                  checked={basicData.priceRanges.low} 
                  onChange={handlePriceChange}
                  className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500"
                />
                <span className="text-sm font-medium text-slate-700">低 (NT$300以下)</span>
              </label>
            </div>
          </div>
        </section>
        ...
        <section className="space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-lg md:text-xl font-bold text-slate-800 flex items-center gap-2">
              ...
              航班資訊 <span className="text-xs font-normal text-slate-500 bg-slate-100 px-2 py-1 rounded ml-2 hidden md:inline">AI 將依機場代碼優化動線</span>
            </h3>
              ...
              <div key={acc.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                <div onClick={() => toggleAccommodation(acc.id)} className="p-4 flex items-center justify-between cursor-pointer hover:bg-slate-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div><div className="font-bold text-slate-800 text-sm md:text-base">{acc.name || '新住宿地點'}</div><div className="text-xs text-slate-500 flex items-center gap-1"><MapPin className="w-3 h-3" /> {acc.address || '尚未輸入地址'}</div></div>
                  </div>
                  <div className="flex items-center gap-2">
  ...
  const renderResult = () => {
    ...
    return (
      <div className="max-w-6xl mx-auto space-y-4 md:space-y-8 animate-in fade-in zoom-in-95 duration-500 pb-20">
        {/* Header Card (手機版 padding 縮小) */}
        <div className="bg-white/90 backdrop-blur-md p-5 md:p-8 rounded-3xl shadow-lg border border-white/50 relative overflow-hidden">
           ...
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6 relative z-10">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-end">
              <button 
                ...
                className={`p-3 md:p-4 rounded-full transition-all shadow-md hover:scale-110 active:scale-95 ${isSaved ? 'bg-red-50 text-red-500 ring-2 ring-red-100' : 'bg-white text-slate-400 hover:text-red-500'}`}
                title={isSaved ? "取消儲存" : "儲存此規劃"}
              >
                <Heart className={`w-5 h-5 md:w-6 md:h-6 ${isSaved ? 'fill-current' : ''}`} />
              </button>
              <button onClick={() => setStep('input')} className="px-4 py-2 md:px-6 md:py-3 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 transition-colors text-sm md:text-base">
                重新規劃
              </button>
            </div>
          </div>
        ...
        {/* Day Tabs */}
        <div className="flex overflow-x-auto pb-4 gap-3 md:gap-4 scrollbar-hide px-2 snap-x">
          {itineraryData.days.map((day, index) => (
            <button
              key={index}
              onClick={() => setActiveTab(index)}
              className={`snap-center flex-shrink-0 px-6 py-3 md:px-8 md:py-4 rounded-2xl transition-all duration-300 border-2 relative overflow-hidden group ${
                activeTab === index 
        ...
        {/* Timeline Content (針對手機版大幅調整間距與字體) */}
                  ...
                  <div className="flex-1 bg-white border border-slate-100 rounded-2xl p-4 md:p-6 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    <div className="flex flex-col md:flex-row justify-between items-start mb-3 md:mb-4 gap-3 md:gap-4">
                      ...
                      <a 
                        ...
                        target="_blank" 
                        rel="noreferrer" 
                        className="flex items-center gap-1 md:gap-2 text-blue-500 hover:text-blue-600 hover:bg-blue-50 px-3 py-1.5 md:px-4 md:py-2 rounded-full transition-colors bg-white border border-blue-100 shadow-sm text-xs md:text-sm"
                      >
                    ...
                    {item.transport_detail && (
                      <div className="bg-slate-50 p-3 md:p-4 rounded-xl mb-3 md:mb-4 flex items-start gap-3 md:gap-4 border border-slate-100">
                        <div className="bg-white p-2 rounded-full shadow-sm shrink-0"><Train className="w-4 h-4 text-slate-500" /></div>
                    ...
                    {item.warnings_tips && (
                      <div className="bg-amber-50 border border-amber-100 p-3 md:p-4 rounded-xl mb-3 md:mb-4 flex items-start gap-3 md:gap-4">
                        <div className="bg-white p-2 rounded-full shadow-sm shrink-0"><AlertTriangle className="w-4 h-4 text-amber-500" /></div>
